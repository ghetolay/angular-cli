...
function build(packagesToBuild, opts, logger) {
    ...
    return Promise.resolve()
        ...
        fs.writeFileSync(output, build_schema_1.buildSchema(input, logger), { encoding: 'utf-8' });
    })
        .then(() => logger.info('Compiling packages...'))
        .then(() => {
        const packagesLogger = new logger_1.Logger('packages', logger);
        ...
        .then(() => {
        const toolsLogger = new logger_1.Logger('packages', logger);
        ...
        const licenseLogger = new logger_1.Logger('license', logger);
        ...
        const tarLogger = new logger_1.Logger('license', logger);
        return Promise.all(Object.keys(packages).map(pkgName => {
            const pkg = packages[pkgName];
...
//# sourceMappingURL=/home/ghetolay/git/angular-cli/src/build.js.map